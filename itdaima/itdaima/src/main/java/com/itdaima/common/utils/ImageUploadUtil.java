...
public class ImageUploadUtil {
            ...
            while (iter.hasNext()) {
                ...
                if (file != null && file.getSize()< 1024*1024*2) {
                    ...
                    if (myFileName.trim() != "") {
                        ...
                        //创建略缩图
                        ImageCompressUtil.saveMinPhoto(realPathDirectory + "\\" + fileName,
                                realPathDirectory + "\\" + millis +"_thumb"+suffix, 120, 1d);
                        //实际要显示的图片   xxx x2.jpg
                        ImageCompressUtil.zipImageFile(realPathDirectory + "\\" + fileName, realPathDirectory + "\\" + millis +"_size"+suffix);
     ...
     * @param response
     * @param DirectoryName
     *            文件上传目录：比如upload(无需带前面的/) uploadImage/..
     * @throws IOException
     */
    public static void ckeditor(HttpServletRequest request, HttpServletResponse response, String DirectoryName)
        ...
        // 结合ckeditor功能
        // imageContextPath为图片在服务器地址，如uploadImage/123.jpg,非绝对路径
        //获取当前日期(年月)
        String yearMonth = DateUtils.formatDate(new Date(), "yyyyMM");
        String imageContextPath = request.getContextPath() + "/" + DirectoryName + "/" + yearMonth + "/" + fileName;
